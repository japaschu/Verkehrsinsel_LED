...
LPD8806 strip = LPD8806(480/2, dataPin, clockPin);
int colorX = 0;
int colorY = 0;
int maxX, maxY, minX;
int i= 0;
...
void loop() {
//  int switchOnOff = analogRead(A0);
  // print out the value you read:
  //Serial.println("X");
  //Serial.println(sensorX);
 
  //strip.Color(1,colorY,colorX);
//  if(sensorX > maxX){
//    maxX = sensorX;
//  }
//  
...
//  Serial.println(switchOnOff);
  ...
  //colorX = colorX + sensorX/(maxY)*127;
//  //Serial.println(sensorX);
//  i = 2;
//  strip.setPixelColor(i, strip.Color(0,1,colorX));
//  strip.show();
  //scanner(max(127,sensorX),60,0, 1);        // red, slow
  ...
  //colorWipe(strip.Color(0,0,100), 10);        // black
  ...
  // Color sparkles
  dither(strip.Color(252, 109, 0), 40);       // orange, slow
  //dither(strip.Color(100,110, 100),10);       // cyan, slow
  //dither(strip.Color(100,0, 0),1);       // green, slow
  //dither(strip.Color(0,100, 0),1);
  ...
  scanner(127,60,0,0);        // red, slow
  //scanner(0,0,127, 10);        // blue, fast

  // Wavy ripple effects
  //wave(strip.Color(127,0,0), 4, 10);        // candy cane
  wave(strip.Color(93, 4, 133), 2, 40);        // icy

  //dither(strip.Color(100,110, 100),10);
...
void rainbowCycle(uint8_t wait) {
  ...
  for (j=0; j < 384 * 5; j++) {     // 5 cycles of all 384 colors in the wheel
    for (i=0; i < strip.numPixels(); i++) {
      ...
      LEDValue = Wheel(((i * 384 / strip.numPixels()) + j) % 384);
...
void rainbowCycle2(uint8_t wait) {
  ...
  for (j=0; j < 384 * 5; j++) {     // 5 cycles of all 384 colors in the wheel
    for (i=0; i < strip.numPixels(); i++) {
      ...
      // Then add in j which makes the colors go around per pixel
      // the % 384 is to make the wheel cycle around
      strip.setPixelColor(i, Wheel(((i * 384 / strip.numPixels()) + j) % 768));
